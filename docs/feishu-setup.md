...
> **One-click onboarding**: when neither the config nor the environment variables provide credentials, the plugin enters an onboarding flow instead of failing: it prints a QR-code / link (valid ~10 minutes) in the `dsh web` log. Scan it with Feishu and the bot app is created automatically with the permissions and event subscriptions pre-set. The returned credentials are written to the **DSH credential store** — the source of truth: the next startup's `injectSecrets` fills the channel config from it, so onboarding does not run again and the Web settings pane reports the channel as configured. A copy is still mirrored, best-effort, to the legacy `$DSH_HOME/.dsh-connect/feishu-credentials.json` (permissions `0600`) for older builds and downgrades. Onboarding also needs a human at a terminal: with `onboarding: false`, or when stdout is not a TTY (the normal headless `dsh web` boot), the flow is skipped with a warning instead of parking a timer — configure the app from the Web settings pane (Settings → Channels) in that case.
