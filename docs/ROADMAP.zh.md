...
> 状态：基于 2026-08-24 的 v0.6.7 代码基线分析后制定；已按计划推进。（**当前代码基线已是 `dsh-connect` 0.9.0**，仓库结构见下方说明。）
> 进度：
...
>
> **计数说明**：上述「5 包」「95 个单测」只描述当时的拆分多包结构。0.8.0 起仓库已收敛为**唯一的 `dsh-connect` 包**（`packages/connect`，见 `CHANGELOG.md` 与 `docs/all-in-one-and-web-settings.md`），当前为单包 **349 项测试全过**（DSH `0.1.5-rc.2`）。
...
> **与实际的偏差**：0.8.0 / 0.8.1 实际交付的是**单包化（方案 B，全部通道吸收进 `dsh-connect`）**与 Web 设置页收尾，本节的 C1–C6 **一条都没有实现**，仍是候选清单（C5 的钉钉按钮卡片菜单、C6 的可观测性均可复核：代码中尚无相应实现）。
