...
> **一键开通**：当配置和环境变量都没有提供凭据时，插件不会报错退出，而是进入扫码注册流程：在 `dsh web` 日志中打印一个二维码/链接（约 10 分钟有效）。用飞书扫码后，机器人应用会自动创建并预置好权限与事件订阅。返回的凭据写入 **DSH 凭据库**——它是唯一事实来源：下次启动时 `injectSecrets` 会用它填充渠道配置，因此不会再次进入开通流程，Web 设置页也会把该渠道显示为「已配置」。同时会尽力往旧的 `$DSH_HOME/.dsh-connect/feishu-credentials.json`（权限 `0600`）镜像一份，供老版本或降级使用。另外，开通流程需要有人在终端前：若设置了 `onboarding: false`，或 stdout 不是 TTY（`dsh web` 无头启动的常态），流程会被跳过并打印警告，此时请改用 Web 设置页（设置 → 通道）配置应用。
