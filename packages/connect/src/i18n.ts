...
const zh: Messages = {
  ...
  welcomeBody: (workdir) =>
    [
      `我是接入 DeepSeek Harness 的飞书助手。当前工作区：\`${workdir}\``,
      "直接发消息即可开始任务，也可以使用这些命令：",
      "- `/menu` 打开主菜单（工作区 / 会话 / 设置 / 压缩等）",
      "- `/help` 查看全部命令",
      "- `/status` 查看会话状态与上下文占用",
      "- `/compact` 上下文占用高时压缩",
  ...
  errorAdvicePermission:
    "可能是权限不足。请检查：飞书应用的权限（开发者后台 → 权限管理，修改后需重新发布版本）、DSH 沙箱与工作区的访问权限。",
  ...
  workdirField: (workdir) => `当前工作区：${workdir}`,
  ...
  workdirSetting: (workdir) => `当前工作区：${workdir}`,
  ...
  noSessionsInWorkdir: (workdir) => `当前工作区下暂无历史会话：\n${workdir}\n\n可发送消息开始新对话，或 /dir 切换工作区。`,
  historySessions: (n, list) => `当前工作区的历史会话（${n}）：\n${list}\n\n点开主菜单「💬 切换对话」可选择并切换到对应会话。`,
  ...
  helpStatus: "查看会话状态、模型、当前工作区、排队数",
  helpTask: "查看当前任务清单（todo）",
  helpChat: "列出对话，点选切换或新建",
  helpDir: "列出工作区，点选切换（或 `/dir <绝对路径>` 手动指定）",
  helpWorkspace: "新建工作区",
  helpPlugins: "查看插件清单",
...
const en: Messages = {
  ...
  menuRoot: "Main menu",
  menuWorkspace: "Switch workspace",
  ...
  welcomeBody: (workdir) =>
    [
      `I'm a Feishu assistant powered by DeepSeek Harness. Current workspace: \`${workdir}\``,
      "Just send a message to start a task, or use these commands:",
      "- `/menu` open the main menu (workspace / sessions / settings / compact…)",
  ...
  errorAdvicePermission:
    "This looks like a permission problem. Check the Feishu app permissions (Developer Console → Permissions; release a new version after changes) and the DSH sandbox / workspace access.",
  ...
  workdirField: (workdir) => `Current workspace: ${workdir}`,
  ...
  workdirSetting: (workdir) => `Current workspace: ${workdir}`,
  ...
  noSessionsInWorkdir: (workdir) => `No historical sessions in the current workspace:\n${workdir}\n\nSend a message to start a new conversation, or use /dir to switch workspaces.`,
  historySessions: (n, list) => `Historical sessions in the current workspace (${n}):\n${list}\n\nOpen the main menu → "Switch conversation" to pick and switch to a session.`,

  currentDir: "Current workspace",
  ...
  helpStatus: "session status, model, current workspace, queue length",
  helpTask: "show the current task list (todo)",
  helpChat: "list conversations; tap to switch or create",
  helpDir: "list workspaces; tap to switch (or `/dir <absolute path>` to set manually)",
