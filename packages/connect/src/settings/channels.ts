/**
 * Channel orchestration for dsh-connect: a small, side-effect-free selector
 * that activates a requested subset of channel adapters. Kept separate from the
 * cordis entry so it is unit-testable without pulling any channel SDK.
 * @module dsh-connect/settings/channels
 */

/** Channel adapters the connect plugin can activate. */
...
export function activateChannels<Ctx extends LoggerLike>(
  ...
  for (const name of wanted) {
    ...
    if (apply === undefined) {
      ctx.logger?.warn?.(`connect: unknown channel "${name}"; skipped`);
      continue;
    }
    try {
      ...
      ctx.logger?.warn?.(`connect: channel "${name}" failed to start: ${String(error)}`);
    }
  }
  return started;
}
...
export async function injectSecrets(
  ...
  const out: ChannelsConfig = { ...config };
